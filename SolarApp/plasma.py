...
void main(void)
{
   ...
   float xs = (10*(x+time-400)) / resolution.x;
   float ys = (10*(y+time-300)) / resolution.x;
   float circle = xs*xs + ys*ys;
   float r = sqrt(xs*xs + ys*ys);//*abs(sin(time*4)+2);
   float energy = (1-circle)/(1.25*abs(sin(time*4)));
   gl_FragColor = vec4(1, 1, 2/r, 1/r);
}
'''
